...
fn main() {
    ...
    for bi in 0..2u32 { for ci in 0..2u32 { for di in 0..2u32 { for ni in 0..2u32 { for ti in 0..2u32 {
        let env: Vec<(VarId, Vec<u32>)> = vec![
            (vs[0], vec![bi]), (vs[1], vec![ci]), (vs[2], vec![di]),
            (vs[3], vec![ni]), (vs[4], vec![ti])];
    ...
    fn cnfv_num(c: &alloy_kodkod_rs::cnf::CnfTranslation) -> usize { c.num_vars }

    let mut ip = IpasirSolver::new().unwrap();
    if cnf.num_vars > ip.num_variables() {
        ip.add_variables(cnf.num_vars - ip.num_variables());
    }
    for cl in &cnf.clauses { ip.add_clause(cl); }
    let sat = SatSolver::solve(&mut ip);
    println!("solver SAT={sat}");
    if !sat { return; }

    let mut model: Vec<bool> = vec![false; cnf.num_vars + 1];
    for s in 1..=cnf.num_vars { model[s] = SatSolver::value_of(&ip, s as i64); }
    let val = |r: BoolRef, m: &[bool]| -> bool {
        if r.is_const() { return r.const_value(); }
        let x = m[r.slot() as usize]; if r.sign() { x } else { !x }
    };

    // sanity: every clause satisfied?
    let mut bad = 0;
    for cl in &cnf.clauses {
        if !cl.iter().any(|&l| val(BoolRef(l as i32), &model)) { bad += 1; }
    }
    ...
    for &slot in &reach {
        ...
        let comp = match &nd {
            ...
            BoolNode::Ite{c,t,e} => { let cv=val(*c,&model); if cv {val(*t,&model)} else {val(*e,&model)} }
        };
        if comp != model[slot as usize] {
            viol += 1;
            if viol <= 8 {
                println!("REACHABLE-DEF-VIOLATION slot={slot} node={nd:?} stored={} computed={comp}", model[slot as usize]);
                // which clauses mention it?
                let mut touching = 0;
                for cl in &cnf.clauses {
                    if cl.iter().any(|&l| l.abs() == slot as i64) {
                        touching += 1;
                        if touching <= 6 { println!("    clause {cl:?}"); }
                    }
                }
    ...
    // the lying binding found previously: b=a1,c=a1,d=a0,n=a0,t=a0
    {
        let env: Vec<(VarId, Vec<u32>)> = vec![
            ...
            (vs[3], vec![0]), (vs[4], vec![0])];
        let bref = tr.formula_ref(&arena, body, &env).expect("body");
        println!("binding b=a1 c=a1 d=a0 n=a0 t=a0: body ref handle={} slot={} sign={}", bref.0, bref.slot(), bref.sign());
        fn audit(
            ...
            let pad = " ".repeat(d * 2);
            let sv = if r.is_const() { r.const_value() } else if r.sign() { m[r.slot() as usize] } else { !m[r.slot() as usize] };
            match f.node(r) {
                Some(BoolNode::Var) => {
                ...
                Some(BoolNode::And(kids)) => {
                    ...
                    let mut a = true;
                    for k in kids.iter() { a &= audit(f, *k, m, d + 1); }
                    println!("{}=> AND {} computed={}", pad, r.0, a);
                    if a != sv { println!("{}!! INCONSISTENT", pad); }
                    a
                }
                ...
                None => { println!("{}CONST-OR-DANGLING h={}", pad, r.0); sv }
            }
        }
        ctx.with_factory(|f| {
    ...
    // ---- Which kid of BIG is false, and what clauses cover it? ----
    {
        ...
        println!("big(slot={bs}) kids={} root-sign={}", kids.len(), rootb.sign());
        let unit = cnf.clauses.iter().find(|c| c.len() == 1 && (c[0] == bs as i64 || c[0] == -(bs as i64)));
        println!("root unit clause: {:?}", unit);

        let mut shown = 0;
        for &khandle in &kids {
            let kslot = khandle.unsigned_abs() as usize;
            let kmodel = if khandle > 0 { model[kslot] } else { !model[kslot] };
            if !kmodel && shown < 2 {
                shown += 1;
                println!("--- FALSE kid h={khandle} slot={kslot} node={:?}",
                    ctx.with_factory(|f| f.node(BoolRef(kslot as i32)).cloned()));
                let mut cnt = 0;
                for cl in &cnf.clauses {
                    if cl.iter().any(|&l| l.abs() == kslot as i64) {
                        cnt += 1;
                        if cnt <= 8 { println!("     clause {cl:?}"); }
                    }
                }
