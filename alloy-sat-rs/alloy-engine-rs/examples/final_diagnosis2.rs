...
fn main() {
    ...
    for a in ["a0", "a1"] { uu.insert(&Tuple::from_atoms(&u, &[a]).unwrap()).unwrap(); }
    bounds.bound_exactly(ub, &uu).unwrap();
    let trel = arena.relation("t", 3);
    let full = { let mut t = TupleSet::new(&u, 3).unwrap();
        for x in ["a0","a1"] { for y in ["a0","a1"] { for z in ["a0","a1"] {
            t.insert(&Tuple::from_atoms(&u,&[x,y,z]).unwrap()).unwrap(); }}}
        t };
    ...
    let vs: Vec<VarId> = ["b","c","d","n","t"].iter().map(|n| arena.variable(n)).collect();
    ...
    let env: Vec<(VarId, Vec<u32>)> = vec![
        (vs[0], vec![1]), (vs[1], vec![1]), (vs[2], vec![0]),
        (vs[3], vec![0]), (vs[4], vec![0])]; // b=c=a1, d=a0, n=t=a0
    ...
    println!("body bref slot={} sign={}", bref.slot(), bref.sign());
    {
        ...
        println!("na h={} node={:?}; conc h={} node={:?}",
            naref.0, tr.ctx.with_factory(|f| f.node(naref).cloned()), cref.0,
    ...
    // explicit per-mask table
    {
        ...
        // SIGNED TREE of r_noimg (with model values from idx3-probe: only s4=T)
        {
            ...
            fn dv(f: &alloy_kodkod_rs::BoolFactory, r: alloy_kodkod_rs::bool::BoolRef,
                  m: &[bool], d: usize) -> bool {
                let pad = " ".repeat(d * 2);
                if r.is_const() { println!("{pad}const({}) h={}", r.const_value(), r.0); return r.const_value(); }
                let sv = if r.sign() { m[r.slot() as usize] } else { !m[r.slot() as usize] };
                match f.node(r) {
                    Some(BoolNode::Var) => { println!("{pad}VAR h={} val={sv}", r.0); sv }
                    Some(BoolNode::And(k)) => {
                        println!("{pad}AND h={} kids={:?}", r.0, k.iter().map(|x| x.0).collect::<Vec<_>>());
                        let mut a = true;
                        for x in k.iter() { a &= dv(f, *x, m, d + 1); }
                        a
                    }
                    Some(BoolNode::Or(k)) => {
                        println!("{pad}OR h={} kids={:?}", r.0, k.iter().map(|x| x.0).collect::<Vec<_>>());
                        let mut a = false;
                        for x in k.iter() { a |= dv(f, *x, m, d + 1); }
                        a
                    }
                    Some(BoolNode::Ite { c, t, e }) => {
                        ...
                        let cv = dv(f, *c, m, d + 1);
                        if cv { dv(f, *t, m, d + 1) } else { dv(f, *e, m, d + 1) }
                    }
                    None => { println!("{pad}DANGLING h={}", r.0); sv }
                }
            }
        ...
        // STRUCTURAL matrix dump for this binding
        {
            ...
            for (i, v) in m_jb.iter() { println!("   pos{} h{}", i, v.0); }
            println!("M_img cells:");
            for (i, v) in m_img.iter() { println!("   pos{} h{}", i, v.0); }
        }
        ...
        for m in 0..16u32 {
            for i in 0..8usize { model[i+1] = (m >> i) & 1 == 1; }
            let (v_noimg, v_add, v_del, v_conc, v_ante, v_na, v_body) = tr.ctx.with_factory(|f| (
                f.eval(r_noimg, &model), f.eval(r_add, &model), f.eval(r_del, &model),
                f.eval(r_conc, &model), f.eval(r_ante, &model), f.eval(r_na, &model),
                f.eval(bref, &model)));
            println!("mask{:3} noImg={:5} addEq={:5} delEq={:5} conc={:5} ante={:5} na={:5} BODY={}",
                m,
                v_noimg, v_add, v_del, v_conc, v_ante, v_na, v_body);
        }
    }
    ...
    if let Some(&m) = falsifying.first() {
        ...
        for i in 0..8usize {
            if (m >> i) & 1 == 1 {
                let x = i / 4; let y = (i / 2) % 2; let z = i % 2;
                println!("   (a{}, a{}, a{})", x, y, z);
            }
        }
        ...
        for i in 0..8usize {
            if (m >> i) & 1 == 1 {
                let names = ["a0", "a1"];
                let x = i / 4; let y = (i / 2) % 2; let z = i % 2;
                ts.insert(&Tuple::from_atoms(&u, &[names[x], names[y], names[z]]).unwrap()).unwrap();
            }
        }
        ...
        println!("pieces: noImg={} addEq={} delEq={} conc={} ante={}",
            ev_c(r_noimg), ev_c(r_add), ev_c(r_del), ev_c(r_conc), ev_c(r_ante));
        println!("intended: noImg=T addEq=F delEq=T conc=T ante=F");
        // signed tree dump of the ANTE gate
        fn dumpv(f: &alloy_kodkod_rs::BoolFactory, r: alloy_kodkod_rs::bool::BoolRef,
                 m: &[bool], d: usize) -> bool {
            use alloy_kodkod_rs::bool::BoolNode;
            let pad = " ".repeat(d * 2);
            if r.is_const() { println!("{pad}const({}) h={}", r.const_value(), r.0); return r.const_value(); }
            ...
            match f.node(r) {
                ...
                Some(BoolNode::And(kids)) => {
                    println!("{pad}AND h={} kids={:?}", r.0, kids.iter().map(|k| k.0).collect::<Vec<_>>());
                    let mut a = true;
                    for k in kids.iter() { a &= dumpv(f, *k, m, d + 1); }
                    println!("{pad}=> AND {} = {a}", r.0);
                    if a != sv { println!("{pad}!! MISMATCH stored={sv}"); }
                    a
                }
                Some(BoolNode::Or(kids)) => {
                    println!("{pad}OR h={} kids={:?}", r.0, kids.iter().map(|k| k.0).collect::<Vec<_>>());
                    let mut a = false;
                    for k in kids.iter() { a |= dumpv(f, *k, m, d + 1); }
        ...
        tr.ctx.with_factory(|f| { dumpv(f, r_ante, &model, 1); });
        return;
    }
}
